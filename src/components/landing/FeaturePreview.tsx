...
export function FeaturePreview({ id }: { id: string }) {
  switch (id) {
    case "multi-tab":
      return (
        <div className="flex w-full gap-1 bg-background/40 px-2 pt-2">
          <span className="rounded-t px-2 py-1 text-[10px] font-mono font-semibold text-emerald-400 bg-card border border-b-transparent border-border/60">
            GET /users
          </span>
          <span className="rounded-t px-2 py-1 text-[10px] font-mono text-muted-foreground/60">
            POST /auth
          </span>
          <span className="rounded-t px-2 py-1 text-[10px] font-mono text-muted-foreground/60">
            DELETE /42
      ...
      return (
        <div className="rounded-md bg-[oklch(0.12_0.005_285)] px-3 py-2 font-mono text-xs space-y-0.5">
          ...
          <div>
            ...
            <span className="text-muted-foreground/40">: </span>
            <span className="text-amber-400/80">128</span>
          </div>
        </div>
      );
    case "import-formats":
      return (
        <div className="flex flex-wrap gap-1.5 p-1">
          {["Postman", "Insomnia", "cURL"].map((fmt) => (
            <span
              key={fmt}
              className="rounded border border-border/50 bg-muted/50 px-2 py-1 text-[10px] font-medium text-muted-foreground"
            >
              {fmt}
            </span>
          ))}
      ...
      return (
        <div className="flex items-center gap-1.5 flex-wrap p-1">
          {[
            ...
            <span
              key={m}
              className={`rounded px-2 py-0.5 text-[10px] font-mono font-semibold ${text} ${bg}`}
            >
              {m}
            </span>
          ))}
